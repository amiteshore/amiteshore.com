function LampStack() {
  const title = "Install LAMP Stack on Arch Linux";
  document.title = title;
  return (
    <main>
      <h1>{title}</h1>
      <p>
        <b>L</b>inux, <b>A</b>pache, <b>M</b>ariaDB/<b>M</b>ySQL and <b>P</b>HP
      </p>
      <h2>Installation</h2>
      <hr />
      ...
      <p>
        ...
        <code>/etc/httpd/conf/httpd.conf</code> so that I can use any directory
        to develop my project instead of the default <code>/srv/http</code>{" "}
        directory (below both lines should have the same path):
      </p>
      <pre>
      ...
      <p>Change permissions so that Apache can access the directories/files:</p>
      <pre>
        # chmod o+x ~
        <br />
        # chmod o+x ~/project
